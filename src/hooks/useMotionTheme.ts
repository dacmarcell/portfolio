...
export function useMotionTheme(): MotionPreset {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("palettechange", onChange as EventListener);
  }, []);

  return useMemo(() => {
    switch (palette) {
      case "summer":
        return { enterDuration: 0.28, exitDuration: 0.2, step: 0.04, ease: "easeOut" };
      case "winter":
        return { enterDuration: 0.5, exitDuration: 0.35, step: 0.06, ease: [0.22, 1, 0.36, 1] };
      case "mono":
        return { enterDuration: 0.32, exitDuration: 0.25, step: 0.05, ease: "easeInOut" };
      default:
        return { enterDuration: 0.35, exitDuration: 0.25, step: 0.05, ease: "easeOut" };
    }
  }, [palette]);
}
